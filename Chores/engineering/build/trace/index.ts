...
export function createSpan(name: string, parentTraceResult?: TraceResult): Span {
  ...
  const span: Span = {
    ...
    traceChildAsync: <T>(name: string, fn: (span: Span) => T | Promise<T>): Promise<T> =>
      traceChild(name).traceAsyncFn(fn),
    traceChildPromise: <T>(name: string, promise: Promise<T>): Promise<T> =>
      traceChild(name).tracePromise(promise),
  };
...
export function task(importMetaMain: boolean, importMetaPath: string) {
  return <T>(
    ...
    return async (span?: Span) => {
      if (span) {
        return span.traceChildAsync(taskName, childSpan =>
          fn(childSpan, onCleanup).finally(() => cleanup())
        );
      }
...
export async function whyIsNodeRunning() {
  if (isCI && process.env.RUNNER_DEBUG === '1') {
    try {
      ...
      // why-is-node-running is optional, only used in debug mode
      console.log('why-is-node-running not installed, skipping debug output');
    }
  }
}
...
function printTree(
  initialTree: TraceResult,
  printNode: (node: TraceResult, branch: string) => string
) {
  function printBranch(
    tree: TraceResult,
    branch: string,
    isGraphHead: boolean,
    isChildOfLastBranch: boolean
  ) {
