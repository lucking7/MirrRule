...
export const buildRejectDomainSetEnhanced = task(
  ...
  // 创建不同用途的 Trie 树
  const mainTrie = new HostnameSmolTrie(); // 主规则
  const extraTrie = new HostnameSmolTrie(); // 额外规则
  const phishingTrie = new HostnameSmolTrie(); // 钓鱼规则
  ...
  for (const [url, mirrors, includeAllSubDomain, allowEmpty] of HOSTS) {
    ...
    for (const domain of result.domains) {
      mainTrie.add(domain, domain.startsWith('.'));
    }
  ...
  for (const [url, mirrors, includeAllSubDomain, allowEmpty] of HOSTS_EXTRA) {
    const result = await processHosts(url, mirrors, includeAllSubDomain, allowEmpty || false);
    for (const domain of result.domains) {
      extraTrie.add(domain, domain.startsWith('.'));
    }
    stats.hosts.total += result.stats.total;
    stats.hosts.invalid += result.stats.invalid;
  }
  ...
  for (const domain of phishingDomains) {
    phishingTrie.add(domain, domain.startsWith('.'));
  ...
  for (const [url, mirrors, includeThirdParty] of ADGUARD_FILTERS) {
    ...
    // 添加到主 Trie
    for (const domain of result.domains) {
      mainTrie.add(domain, false);
    }
    for (const suffix of result.domainSuffixes) {
      mainTrie.add(suffix, true);
    }
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (ruleType === 'DOMAIN' || ruleType === 'DOMAIN-SUFFIX') {
          ...
          stats.surge.total++;
          mainTrie.add(value, ruleType === 'DOMAIN-SUFFIX');
  ...
  for (const domain of whitelistDomains) {
    mainTrie.whitelist(domain);
    extraTrie.whitelist(domain);
    phishingTrie.whitelist(domain);
  }
  ...
  const mainSuffixes: string[] = [];
  const extraDomains: string[] = [];
  const extraSuffixes: string[] = [];
  const phishingDomains2: string[] = [];
  const phishingSuffixes: string[] = [];

  mainTrie.dump((domain: string, isSubdomain: boolean) => {
    if (isSubdomain) {
      mainSuffixes.push(domain);
    } else {
      mainDomains.push(domain);
    }
  });

  extraTrie.dump((domain: string, isSubdomain: boolean) => {
    if (isSubdomain) {
      extraSuffixes.push(domain);
    } else {
      extraDomains.push(domain);
    }
  });

  phishingTrie.dump((domain: string, isSubdomain: boolean) => {
    if (isSubdomain) {
      phishingSuffixes.push(domain);
    } else {
      phishingDomains2.push(domain);
    }
  });
  ...
  const mainOutput: string[] = [
    ...
    `#   - Main rules: ${mainDomains.length + mainSuffixes.length}`,
    `#   - IP rules: ${optimizedIpCidrs.length + optimizedIpCidr6s.length}`,
    '',
  ];

  // 添加域名规则
  for (const domain of mainDomains.sort()) {
    mainOutput.push(`DOMAIN,${domain}`);
  }
  for (const suffix of mainSuffixes.sort()) {
    mainOutput.push(`DOMAIN-SUFFIX,${suffix}`);
  }
  // 添加关键词规则
  for (const keyword of finalKeywords.sort()) {
    mainOutput.push(`DOMAIN-KEYWORD,${keyword}`);
  }
  // 添加 IP 规则
  for (const cidr of optimizedIpCidrs) {
    mainOutput.push(`IP-CIDR,${cidr},no-resolve`);
  }
  for (const cidr of optimizedIpCidr6s) {
    mainOutput.push(`IP-CIDR6,${cidr},no-resolve`);
  }

  await fs.writeFile(path.join(OUTPUT_DIR, 'block.list'), mainOutput.join('\n'), 'utf-8');

  // 额外规则文件
  const extraOutput: string[] = [
    "# Sukka's Surge Reject Extra Rules",
    '# Additional blocking rules with lower confidence',
    '#   - pgl.yoyo.org serverlist (blocks t.co)',
    '#   - someonewhocares.org hosts',
    '#   - AdGuard CNAME trackers',
    `# Updated: ${new Date().toISOString()}`,
    `# Total rules: ${extraDomains.length + extraSuffixes.length}`,
    '',
  ];

  for (const domain of extraDomains.sort()) {
    extraOutput.push(`DOMAIN,${domain}`);
  }
  for (const suffix of extraSuffixes.sort()) {
    extraOutput.push(`DOMAIN-SUFFIX,${suffix}`);
  }

  await fs.writeFile(path.join(OUTPUT_DIR, 'block_extra.list'), extraOutput.join('\n'), 'utf-8');

  // 钓鱼域名文件
  const phishingOutput: string[] = [
    "# Sukka's Surge Phishing Protection Rules",
    '# Domains identified as phishing or malicious',
    '#   - Phishing.army blocklist',
    '#   - Durablenapkin scamblocklist',
    '#   - Phishing filter pages.dev',
    `# Updated: ${new Date().toISOString()}`,
    `# Total rules: ${phishingDomains2.length + phishingSuffixes.length}`,
    '',
  ];

  for (const domain of phishingDomains2.sort()) {
    phishingOutput.push(`DOMAIN,${domain}`);
  }
  for (const suffix of phishingSuffixes.sort()) {
    phishingOutput.push(`DOMAIN-SUFFIX,${suffix}`);
  }

  await fs.writeFile(path.join(OUTPUT_DIR, 'phishing.list'), phishingOutput.join('\n'), 'utf-8');

  // 打印最终统计
  console.log(picocolors.green('\n✅ 构建完成！'));
  console.log(picocolors.cyan('📊 最终统计:'));
  console.log(`  - 主规则: ${mainDomains.length + mainSuffixes.length} 条`);
  console.log(`  - 额外规则: ${extraDomains.length + extraSuffixes.length} 条`);
  console.log(`  - 钓鱼规则: ${phishingDomains2.length + phishingSuffixes.length} 条`);
