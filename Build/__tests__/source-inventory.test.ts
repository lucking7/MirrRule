...
describe('structured source inventory', () => {
  it('enumerates every network source role in stable order', () => {
    const inventory = createSourceInventory(
      ...
      [{ name: 'mirror', repositories: [{ repo: 'owner/repo', outputDir: '/tmp', allowedTypes: [] }], extraDownloads: [{ url: 'https://extra.test/a', outputPath: '/tmp/a' }] }]
    );
    assert.deepEqual(inventory.map(source => source.role),
      ['fallback', 'mirror-extra', 'mirror-repository', 'primary', 'special-source']);
  ...
  it('enumerates the production configuration without duplicate identities', () => {
    ...
    for (const role of ['primary', 'special-source', 'mirror-repository', 'mirror-extra']) {
