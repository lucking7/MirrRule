...
const dispatcher = new Agent({ allowH2: true }).compose(
  ...
  interceptors.retry({
    ...
    retry(err, { state, opts }, cb) {
      ...
      const retryAfterHeader = (headers as Record<string, string> | null | undefined)?.[
        'retry-after'
      ];
      let retryAfter = -1;
      if (retryAfterHeader) {
      ...
      if (retryAfter > 0) {
        retryTimeout = Math.min(retryAfter, maxTimeout);
      } else {
        const exponentialFactor = timeoutFactor ** (state.counter - 1);
        const exponentialBackoff = minTimeout * exponentialFactor;
        retryTimeout = Math.min(exponentialBackoff, maxTimeout);
      }
