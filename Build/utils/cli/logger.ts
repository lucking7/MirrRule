...
export class Logger {
  ...
  debug(message: string, ...args: unknown[]): void {
    if (this.config.minLevel <= LogLevel.DEBUG) {
      console.log(
        this.getTimestamp() + this.getPrefix() + picocolors.gray(`🔍 ${message}`),
        ...args
      );
    }
  }
  ...
  success(message: string, ...args: unknown[]): void {
    if (this.config.minLevel <= LogLevel.SUCCESS) {
      console.log(
        this.getTimestamp() + this.getPrefix() + picocolors.green(`✅ ${message}`),
        ...args
      );
    }
  }
  ...
  warn(message: string, ...args: unknown[]): void {
    if (this.config.minLevel <= LogLevel.WARN) {
      console.log(
        this.getTimestamp() + this.getPrefix() + picocolors.yellow(`⚠\uFE0F  ${message}`),
        ...args
      );
    }
  }
  ...
  error(message: string, ...args: unknown[]): void {
    if (this.config.minLevel <= LogLevel.ERROR) {
      console.error(
        this.getTimestamp() + this.getPrefix() + picocolors.red(`❌ ${message}`),
        ...args
      );
    }
  }
