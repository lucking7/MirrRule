...
export class WildcardAnalyzer {
   ...
   */
  static analyzeWildcard(this: void, pattern: string): WildcardAnalysis {
    ...
    if (WildcardAnalyzer.isSimpleSuffixPattern(pattern)) {
      result.isSimpleSuffix = true;
      result.extractedSuffix = pattern.slice(2); // 移除 "*."
    } else if (WildcardAnalyzer.hasWildcard(pattern)) {
   ...
   * 模式：以"*."开头，不包含其他通配符
   */
  private static isSimpleSuffixPattern(this: void, pattern: string): boolean {
    return pattern.startsWith('*.') && !pattern.includes('?') && pattern.split('*').length === 2;
  }

  /**
   * 检查是否包含通配符
   */
  private static hasWildcard(this: void, pattern: string): boolean {
   ...
   * @returns 是否为有效域名
   */
  static isValidDomain(this: void, domain: string): boolean {
