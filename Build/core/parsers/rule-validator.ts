...
import type {
  ProxyPlatform,
  PolicyType,
  LogicalOperator } from '../../constants/rule-formats';
import {
  RuleType,
  RuleParameter,
...
export class RuleValidator {
   ...
   */
  static validateRule(parsed: ParsedRule, targetPlatform: ProxyPlatform): RuleValidationResult {
    ...
    if (parsed.platform && parsed.platform !== targetPlatform) {
      result.warnings.push(
        `规则格式为 ${parsed.platform}，但目标平台为 ${targetPlatform}`
      );
    }

    // 检查参数支持
    if (parsed.parameters) {
      const paramErrors = this.validatePlatformParameterSupport(
        parsed.parameters,
        targetPlatform
      );
    ...
    if (parsed.isLogical && parsed.logicalOperator) {
      const operatorError = this.validateLogicalOperator(
   ...
   * @returns 过滤后的参数列表
   */
  static filterRuleSetParameters(parameters: RuleParameter[], ruleType: RuleType): string[] {
   ...
   * 检查是否为IP类规则
   */
  private static isIpRule(ruleType: RuleType): boolean {
   ...
   * 检查是否为域名类规则
   */
  private static isDomainRule(ruleType: RuleType): boolean {
    return [
      RuleType.DOMAIN,
