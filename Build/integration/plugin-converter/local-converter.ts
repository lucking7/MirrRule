...
interface RewriteRule {
  pattern: string;
  replacement: string;
  type: 'reject' | 'reject-dict' | 'reject-array' | 'reject-200' | 'reject-img' | 'reject-tinygif' | 'mock' | 'redirect' | '302' | '307' | 'header' | 'header-rewrite' | 'jq';
  comment?: string;
  mockData?: {
    dataType?: string;
    statusCode?: number;
...
class LoonPluginParser {
  ...
  private static parseArgument(line: string): Argument | null {
    const regex = /^([^=]+?)\s*=\s*(\w+)\s*,\s*(.+?)\s*,\s*tag\s*=\s*([^,]+)(?:\s*,\s*desc\s*=\s*(.*))?$/;
  ...
  private static parseHeaderRewrite(line: string): HeaderRewriteRule | null {
    const match = line.match(/^(.+?)\s+(response-)?(header-(del|add|replace|replace-regex))\s+(.+)$/);
    if (!match) return null;
  ...
  private static parseRewrite(line: string): RewriteRule | null {
    ...
    if (line.includes(' mock-response-body ')) {
      ...
      if (match) {
        ...
        const statusCode = parseInt(LocalPluginConverter.findParamValue(params, 'status-code') || '200');
    ...
    const jqRegex = /^(.+?)\s+(response-body-json-jq|http-response-json-jq|response-json-jq|request-body-json-jq|http-request-json-jq)\s+(.+)$/i;
...
class LocalPluginConverter {
  ...
  private dedupeMapLocal(list: RewriteRule[]): RewriteRule[] {
    ...
    for (const rule of list) {
      ...
      const key = `${rule.pattern}|${rule.mockData?.dataType || '-'}|${rule.mockData?.statusCode || 200}`;
  ...
  private loonToSurge(loon: LoonPlugin): SurgeModule {
    ...
    for (const rewrite of loon.rewrites) {
      ...
      } else if (rewrite.type === 'reject-200' || rewrite.type === 'reject-img' || rewrite.type === 'reject-tinygif') {
  ...
  private generateSurgeOutput(surge: SurgeModule): string {
    ...
    if (surge.headerRewrites.length > 0) {
      ...
      for (const headerRewrite of surge.headerRewrites) {
        ...
        lines.push(`${headerRewrite.type} ${headerRewrite.pattern} ${headerRewrite.action} ${formattedParams}`);
      }
      lines.push('');
    }
    ...
    if (surge.mapLocal.length > 0) {
      ...
      for (const mapLocal of surge.mapLocal) {
        ...
        if (mapLocal.mockData?.statusCode) parts.push(`status-code=${mapLocal.mockData.statusCode}`);
    ...
    if (surge.scripts.length > 0) {
      ...
      for (const script of surge.scripts) {
        const scriptName = script.enableParam ? `{{{${script.enableParam}}}}${script.name}` : script.name;
  ...
  private generateArgumentsLine(args: Argument[]): string {
    ...
    const parts = Array.from(uniqueArgs.values()).map(arg => {
      ...
      const value = typeof arg.defaultValue === 'boolean' ? arg.defaultValue : `"${arg.defaultValue}"`;
 ...
 * 使用镜像系统缓存插件文件
 */
export async function convertPluginLocally(plugin: PluginInfo, forceUpdate = false): Promise<LocalConversionResult> {
 ...
 * 批量本地转换插件
 */
export async function convertPluginsLocallyBatch(plugins: PluginInfo[], forceUpdate = false): Promise<LocalConversionResult[]> {
  ...
  console.log(picocolors.green(`[Local Converter] Converted ${successCount}/${plugins.length} plugins`));
