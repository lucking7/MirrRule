 ...
 * 下载并镜像 Loon 插件
 */
export async function mirrorPlugin(plugin: PluginInfo): Promise<{ success: boolean; content?: string; error?: string }> {
 ...
 * 优先使用镜像，不存在则下载并镜像
 */
export async function getPluginContent(plugin: PluginInfo, forceUpdate = false): Promise<{ success: boolean; content?: string; error?: string }> {
  // 检查是否已镜像（且不强制更新）
  ...
  // 下载并镜像
  return await mirrorPlugin(plugin);
}

/**
 * 批量镜像插件
 */
export async function mirrorPluginsBatch(plugins: PluginInfo[], forceUpdate = false): Promise<{
  total: number;
  mirrored: number;
  cached: number;
  failed: number;
