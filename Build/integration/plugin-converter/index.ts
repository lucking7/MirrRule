...
import { convertPluginsBatch, waitForScriptHub } from './script-hub-client';
import {
  extractScriptUrls,
...
export async function convertAndMirrorPlugins(waitForService = false): Promise<ConversionResult[]> {
  ...
  const conversionResults = await convertPluginsBatch(plugins);

  const results: ConversionResult[] = [];
  const allScripts: string[] = [];

  for (const { plugin, content } of conversionResults) {
    if (typeof content === 'string') {
      // 提取脚本
      const scripts = extractScriptUrls(content);

      // 从内容中提取模块名称
      const moduleName = extractModuleName(content, plugin.name);
      ...
      console.log(picocolors.gray(`  ✓ ${plugin.name} → ${fileName}`));
  ...
  // 3. 提取所有脚本
  console.log(picocolors.cyan('\n[Step 3/4] Extracting JavaScript URLs...\n'));
  ...
  if (scriptStats.needMirror > 0) {
    console.log(picocolors.cyan('\n[Step 4/4] Mirroring JavaScript files...\n'));
    ...
    // 5. 更新 sgmodule 文件中的 URL
    if (mirrorResult.mirrored > 0) {
      console.log(picocolors.cyan('\n[Step 5/4] Updating sgmodule files with mirror URLs...\n'));
    ...
    console.log(picocolors.gray('\n[Step 4/4] No scripts to mirror - skipping\n'));
  }
