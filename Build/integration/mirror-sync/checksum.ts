 ...
 * @returns 校验和信息数组
 */
export async function calculateMultipleChecksums(filePaths: string[]): Promise<FileChecksum[]> {
  const results = await Promise.all(filePaths.map(path => calculateFileChecksum(path)));
