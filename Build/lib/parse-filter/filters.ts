...
export function processFilterRulesWithPreload(
  ...
  const downloadPromise = fetchAssets(filterRulesUrl, fallbackUrls, true, false);
...
export function parse(
...
): [hostname: string, flag: ParseType] {
  ...
  if (filter) {
    ...
    if (
      ...
      if (_3p) {
        if (_1p || includeThirdParty) {
          // both first party and third party are true
  ...
  if (
    !includeThirdParty &&
    (line.includes('third-party', indexOfDollar + 1) || line.includes('3p', indexOfDollar + 1))
  ) {
    result[1] = ParseType.Null;
    return result;
  }
  ...
  const sliced =
    sliceStart > 0 || sliceEnd < 0
      ? line.slice(sliceStart, sliceEnd === 0 ? undefined : sliceEnd)
      : line;
